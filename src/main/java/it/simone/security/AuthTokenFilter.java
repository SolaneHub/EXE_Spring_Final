...
public class AuthTokenFilter extends OncePerRequestFilter {
	...
	@Override
	// Request e Response della servlet, FilterChain richiama il filtro successivo
	// nella catena o la risorsa finale
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		try {
			...
			if (jwt != null && jwtUtils.validateJwtToken(jwt)) {
				...
				// Crea un WebAuthenticationDetails contenente informazioni sulla request in
				// corso
				authentication.setDetails(new WebAuthenticationDetailsSource().buildDetails(request));
				// Aggiunge l'oggetto Authentication nel SecurityContext in modo che sia
				// utilizzabile dalle successive chiamate HTTP
