...
public class ServiceIndirizzo extends AbstractService<Indirizzo, Long> {
	...
	public ServiceIndirizzo(RepositoryIndirizzo repositoryIndirizzo, RepositoryComune repositoryComune,
			RepositoryProvincia repositoryProvincia) {
	...
	public Indirizzo prepareAndSave(Indirizzo indirizzo) {
		if (indirizzo.getComune() != null && indirizzo.getComune().getProvincia() != null) {
			...
			Optional<Provincia> existingProvinciaOpt = repositoryProvincia.findByNomeAndSigla(provincia.getNome(),
					provincia.getSigla());
			if (existingProvinciaOpt.isEmpty()) {
				throw new RuntimeException("Provincia con Nome '" + provincia.getNome() + "' e Sigla '"
						+ provincia.getSigla() + "' non trovata.");
			}
			...
			if (existingComuneOpt.isEmpty()) {
				throw new RuntimeException("Comune con Nome '" + comune.getNome() + "' e Provincia '"
						+ provincia.getNome() + "' non trovato.");
			}
